...
class TestRectangle(unittest.TestCase):
    ...
    def test_B_Constructor(self):
        ...
        expected_error = "__init__() missing 2 required positional arguments:\
                'width' and 'height'"
        self.assertEqual(actual_error.replace(" ", ""), expected_error.replace(" ", ""))

    def test_C_Constructor2(self):
        '''checks for multiple args'''
        with self.assertRaises(TypeError) as e:
            r = Rectangle(2, 4, 3, 2, 7, 8)
        actual_error = str(e.exception)
        expected = "__init__() takes from 3 to 6 positional arguments but 7 were given"
        self.assertEqual(actual_error.replace(" ", ""), expected.replace(" ", ""))

    def test_D_Id_sync(self):
        '''test if the ids are in sync'''
        r = Rectangle(5, 7)
        self.assertEqual(r.id, getattr(Rectangle, "_Base__nb_objects"))

    def test_E_id_sync(self):
        '''test for more sync'''
        r = Rectangle(5, 6)
        r3 = Rectangle(9, 7, id="foo")
        r4 = Rectangle(7, 2)
        self.assertEqual(r.id +1, r4.id)

    def test_F_attribute(self):
        '''Check attributes'''
        r = Rectangle(5, 6, 1, 3, 5)
        self.assertEqual(r.width, 5)
        self.assertEqual(r.height, 6)
        self.assertEqual(r.x, 1)
        self.assertEqual(r.y, 3)
        self.assertEqual(getattr(r, "id"), 5)
